...
export const PLACEHOLDER_CATALOG: PlaceholderGroup[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { token: 'outrosN2FlowScfm', label: 'Vazão N₂ desalagar DPR/HCR' },
      { token: 'pressaoCavFibop', label: 'Cavidade FIBOP/FDR' },
      { token: 'pressaoHcr', label: 'HCR — estanqueidade (AGMAR)' },
      { token: 'pressaoBoreTest', label: 'Teste bore 2"/4" e CWO' },
      { token: 'pressaoRiserBores', label: 'Riser DB — teste dos bores 4"/2"' },
      { token: 'pressaoRiserCavConexao', label: 'Riser DB — cavidade de conexão (descida)' },
      { token: 'pressaoColunaDpr', label: 'Estanqueidade da coluna de DPR' },
      { token: 'pressaoColunaRiserDb', label: 'Estanqueidade da coluna de riser DB' },
